...
<?php 
    ...
    if (isset($_GET['edit'])) 
        ...
        $id = $_GET['edit'];
        $update = true;
        $eredmeny = mysqli_query($connect, "SELECT * FROM referencia_lektoralas WHERE id=$id");
        
        $a = mysqli_fetch_array($eredmeny);
        $bekezdes = $a['ref_lekt_szoveg'];
        }
?>
...
<body>
    ...
    <?php $results = mysqli_query($connect, "SELECT * FROM referencia_lektoralas"); ?>
...
<div class="container">
    <h1 class="text-center">Referenciák - Lektorálás</h1>
    <table class="table table-bordered">
        ...
        <?php while ($row = mysqli_fetch_array($results)) { ?>
            <tr>
                <td><?php echo $row['ref_lekt_szoveg']; ?></td>
                <td>
                    <a type="button" class="btn btn-warning" href="cms_2_1.php?edit=<?php echo $row['id']; ?>">Szerkesztés</a>
                </td>
                <td>
                    <a type="button" class="btn btn-danger" href="cms_ref_lekt.php?del=<?php echo $row['id']; ?>">Törlés</a>
                </td>
            </tr>
        <?php } ?>
        </tbody>
...
<div class="container">
    <form method="post" action="cms_ref_lekt.php" >
