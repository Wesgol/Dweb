...
<?php  include('cms_szolg_infokt.php'); ?>
...
<?php 
    ...
    if (isset($_GET['edit'])) 
        ...
        $id = $_GET['edit'];
        $update = true;
        $eredmeny = mysqli_query($connect, "SELECT * FROM szolgaltatasok_infokt WHERE id=$id");
        
        $a = mysqli_fetch_array($eredmeny);
        $bekezdes = $a['infokt_szoveg'];
...
<body>
    ...
    <?php $results = mysqli_query($connect, "SELECT * FROM szolgaltatasok_infokt"); ?>
...
<div class="container">
    <h1 class="text-center">Szolgáltatások - Informatikai oktatás</h1>
    <table class="table table-bordered">
        ...
        <?php while ($row = mysqli_fetch_array($results)) { ?>
            <tr>
                <td><?php echo $row['infokt_szoveg']; ?></td>
                <td>
                    <a type="button" class="btn btn-warning" href="cms_1_3.php?edit=<?php echo $row['id']; ?>">Szerkesztés</a>
                </td>
                <td>
                    <a type="button" class="btn btn-danger" href="cms_szolg_infokt.php?del=<?php echo $row['id']; ?>">Törlés</a>
                </td>
            </tr>
        <?php } ?>
        </tbody>
...
<div class="container">
    <form method="post" action="cms_szolg_infokt.php" >
